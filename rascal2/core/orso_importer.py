...
def _ensure_parameter(
    ...
    project.parameters.append(Parameter(name=name, min=pmin, value=pval, max=pmax, fit=True))
    return name
                ...
                if len(resolved) >= 2:
                    ...
                    for li in resolved[1:-1]:
                        lname = _sanitize_name(
                            getattr(li, "original_name", None) or getattr(li.material, "name", None),
                            "Layer",
                        )
                        ...
                        project.layers.append(Layer(name=lname, thickness=t_p, roughness=r_p, SLD_real=s_p))
                        layer_name_stack.append(lname)
                        base_layers_for_custom.append({"name": lname, "thickness": t, "sld": s, "roughness": r})
