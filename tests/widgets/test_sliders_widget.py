...
def slider():
    ...
    class_view = ratapi.ClassList(
        [
            DataModel(name="A", min = 0,value=1,max=100,fit=True,show_priors = False),
            DataModel(name="B", min = 0, value=6,max = 200,fit=True,show_priors = False),
            DataModel(name="C", min = 0,value=18,max=300,fit=True,show_priors = False)
        ]
    )
